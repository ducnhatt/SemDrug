...
# basically for linearization, which converts internal ID to name
def load_and_process_mappings(dataset_name, paths, hetionet_path):
    logger.info("Processing nodes")
    # hetionet nodes have the format: {"kind": "Compound", "identifier": "DB:...", "name": "Aspirin"}
    def preprocess_B_to_dict(B):
        return {f"{item['kind']}::{item['identifier']}": item["name"] for item in B}

    # A is internal mapping ID : external ID / external DB, B is external ID / external DB : name
    # mapping: internal ID:name
    ...
    # load drug (task-specific) node2id 
    ...
    # for pharmaDB, it's already .json
    else:
        drug_dict = load_json(paths[dataset_name]["drug_info"])

    drugs_to_name_dict = {
        key: value_to_name(drug_dict.get(value))
...
def build_graph_from_file(file_path, dataset, add_reverse_edges=False, offset=0):
    ...
    # this creates a set of tuples (d1_id, label_idx+offset, d2_id)
    # for which should be removed: the direct edge d1-d2
...
def load_and_process_dataset(dataset_name, split, paths, debug=False):
    ...
    def process_ddinter_dataset(dataset):
        ...
        if split == "train":
            # convert to tuples "('DDInter644', 'DDInter885')" -> python list ["DDInter644", "DDInter885"]
            dataset = dataset.map(
                lambda example: {
    ...
    def process_drugbank_dataset(dataset):
        logger.info(f"Initial DrugBank dataset columns: {dataset.column_names}")
        ...
        def lookup_node_id(x):
            """
            DrugBank files may store endpoints either as:
            - numeric internal node IDs
            - DrugBank IDs such as DB00316
            """
            if x is None:
                return -1
            ...
            return int(drug_data.get(x, -1))

        def normalize_example(example):
            # ------------------------------------------------------------
            # 1. Normalize endpoint DB IDs
            # ------------------------------------------------------------
            if "drug1_db" not in example:
                if "Drug1_ID" in example:
                    example["drug1_db"] = example["Drug1_ID"]
                elif "drugbank_id_1" in example:
                    example["drug1_db"] = example["drugbank_id_1"]
                elif "drug1_id" in example:
                    # Could already be numeric node id; keep as fallback string
                    example["drug1_db"] = str(example["drug1_id"])

            if "drug2_db" not in example:
                if "Drug2_ID" in example:
                    example["drug2_db"] = example["Drug2_ID"]
                elif "drugbank_id_2" in example:
                    example["drug2_db"] = example["drugbank_id_2"]
                elif "drug2_id" in example:
                    example["drug2_db"] = str(example["drug2_id"])

            # ------------------------------------------------------------
            # 2. Normalize endpoint internal node IDs
            # ------------------------------------------------------------
            if "drug1_id" in example and "drug2_id" in example:
                example["drug1_id"] = lookup_node_id(example["drug1_id"])
                example["drug2_id"] = lookup_node_id(example["drug2_id"])
            elif "drug1_db" in example and "drug2_db" in example:
                example["drug1_id"] = lookup_node_id(example["drug1_db"])
                example["drug2_id"] = lookup_node_id(example["drug2_db"])
            else:
                raise KeyError(
                    "Could not infer DrugBank endpoint columns. "
                    f"Available columns: {list(example.keys())}"
                )

            # ------------------------------------------------------------
            # 3. Normalize label_idx
            # DrugBank is 86-class, usually 0..85.
            # ------------------------------------------------------------
            if "label_idx" in example:
                example["label_idx"] = safe_int(example["label_idx"], default=-1)
            elif "Y" in example:
                example["label_idx"] = safe_int(example["Y"], default=-1)
            elif "label" in example:
                example["label_idx"] = safe_int(example["label"], default=-1)
            elif "output" in example:
                example["label_idx"] = safe_int(example["output"], default=-1)
            else:
                raise KeyError(
                    "Could not infer DrugBank label column. "
                    f"Available columns: {list(example.keys())}"
                )

            # Keep a label field for compatibility with few-shot / downstream code.
            if "label" not in example:
                example["label"] = str(example["label_idx"])

            # Optional compatibility field.
            if "drug_pair" not in example:
                example["drug_pair"] = str((example.get("drug1_db", ""), example.get("drug2_db", "")))

            return example

        dataset = dataset.map(normalize_example, num_proc=1)

        logger.info("DrugBank dataset processed")
        logger.info(f"DrugBank columns after processing: {dataset.column_names}")
