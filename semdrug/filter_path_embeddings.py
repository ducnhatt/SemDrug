...
EMBEDDINGS_ROOT = os.environ.get("SEMDRUG_EMBEDDINGS_ROOT", os.path.join(PROJECT_ROOT, "embeddings"))

PATHS = {
    "drugbank": {
        "inductive": os.path.join(DATA_ROOT, "drugbank", "drugbank_Augmented_KG.txt"),
        ...
        "embeddings": os.path.join(EMBEDDINGS_ROOT, "drugbank_node_embeddings_full.pkl"),
    },
    "ddinter": {
        "inductive": os.path.join(DATA_ROOT, "ddinter", "ddinter_Augmented_KG.txt"),
...
HETIO_NET_PATH = os.path.join(DATA_ROOT, "hetionet", "hetionet-v1.0.json")
RELATIONS_PATH = os.path.join(DATA_ROOT, "relations_dicts_file_all.json")

EMBEDDING_MODEL_NAME = os.environ.get(
    "SEMDRUG_EMBEDDING_MODEL",
...
def coerce_node_id(value, node2id=None):
    """
    Convert endpoint identifiers to integer KG node IDs.

    Processed SemDrug files usually contain integer drug1_id/drug2_id.
    DrugBank files may contain DrugBank IDs such as DB00316 under
    Drug1_ID/Drug2_ID or drug1_db/drug2_db.
    """
    if value is None:
        return None
    ...
    if node2id is not None and value_str in node2id:
        return int(node2id[value_str])

    return None


def get_pair_node_ids(example, dataset_name, node2id=None):
    """
    Return start/end node IDs for all supported tasks.
    """
    if dataset_name == "pharmaDB":
        candidate_pairs = [
            ("drug1_id", "drug2_id"),
    ...
    for left_key, right_key in candidate_pairs:
        if left_key not in example or right_key not in example:
            continue

        left = coerce_node_id(example.get(left_key), node2id=node2id)
        right = coerce_node_id(example.get(right_key), node2id=node2id)

        if left is not None and right is not None:
            return left, right

    raise KeyError(
        f"Could not infer endpoint node IDs for dataset={dataset_name}. "
...
#=======================add new===============
    ...
    if args.add_cold_start_similarity_edges:
        ...
        relation_id_to_name[similarity_relation_id] = "{u} is pharmacologically similar to {v}"
...
#=======================add new===============
    ...
    def short_text(x, max_chars=1000):
    ...
    def build_query_text(example, dataset_name):
        if dataset_name == "pharmaDB":
            ...
            return (
                f"Task: retrieve drug-drug interaction mechanism evidence for {d1} and {d2}.\n"
                f"Goal: find complete KG paths useful for predicting the specific interaction, "
                f"effect, or mechanism between the two drugs.\n"
                f"Prioritize pharmacokinetic and pharmacodynamic mechanisms: CYP metabolism," 
                f"enzyme inhibition or induction, transporters, serum concentration changes, QT prolongation, "
                f"arrhythmia, bleeding risk, anticoagulant or antiplatelet effects, CNS depression, respiratory depression, "
                f"hepatotoxicity, nephrotoxicity, cardiotoxicity, hypotension, "
                f"hyperkalemia, contraindication, shared targets, and strong pharmacologic similarity.\n"
                f"Downweight generic shared mild side effects unless they support the specific interaction mechanism.\n"
                f"Drug 1: {d1}. Description: {short_text(example.get('drug1_desc', ''))}\n"
                f"Drug 2: {d2}. Description: {short_text(example.get('drug2_desc', ''))}"
            )
        return ""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="/workspace/SemDrug/prompt")
