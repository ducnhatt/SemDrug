    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                ...
                f"lines=True error: {e1}\n"
                f"normal JSON error: {e2}"
            )

truth_json = "/workspace/SemDrug/data/drugbank_semantic_mmr_d4_pool64_k10_mmr0.5_delta0.05_coldtop1sim0.9.json"
truth = load_json_table(truth_json)
...
def strip_leading_option_number(text):
...
# Setup
# Setup
# prediction_path = "/workspace/SemDrug/outputs/drugbank_semantic_mmr_d4_pool64_k10_mmr0.5_delta0.05_coldtop1sim0.9_kg/predictions.csv"
prediction_path = "/workspace/SemDrug/outputs/drugbank_semantic_mmr_d3_pool64_k10_mmr0.5_delta0.05_kg/predictions.csv"
# truth_json = os.path.join(DATA_ROOT, "drugbank", "drugbank_test_set.json")
# truth = pd.read_json(truth_json, lines=True)
...
preds["preds"] = (
    ...
    .apply(extract_last_answer)
    .apply(strip_leading_option_number)
)

if len(preds) != len(truth):
    raise ValueError(
        f"Length mismatch: predictions={len(preds)}, truth={len(truth)}"
    )

truth_copy = truth.copy()

if "label_idx" not in truth_copy.columns:
    if "Y" in truth_copy.columns:
        truth_copy["label_idx"] = truth_copy["Y"].astype(int)
    else:
        raise KeyError(
            f"Cannot find label_idx or Y in truth file. Columns: {truth_copy.columns}"
        )

truth_copy["preds"] = preds["preds"].values
