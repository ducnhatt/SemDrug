...
def find_closest_match_without_drug_names2(
    ...
    # )
    model_prediction = strip_leading_option_number(model_prediction)
...
def parse_drugbank_outputs(predictions: list, drug_1_names: list, drug_2_names: list, extract_fn):
    ...
    for i, pred in enumerate(predictions):
        ...
        if isinstance(pred_str, str):
            pred_str = strip_leading_option_number(pred_str)
    ...
    print("Debug rows before saving:", len(debug_df))
    ...
    debug_df.to_json(
        "drugbank_regex_parse_debug.jsonl",
        orient="records",
        lines=True,
        force_ascii=False,
    )
